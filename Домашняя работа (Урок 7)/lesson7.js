...
var livescore; // Результат во время игры
...
function init() {
    ...
    livescore = document.createElement("p");
    livescore.className = "livescore";
    livescore.insertAdjacentHTML("afterBegin", "Ваш результат: " + score);
    wrap.insertBefore(livescore,wrap.children[0]);
...
function move() {
    ...
    if (direction == 'x-') {
        ...
        if (new_unit === undefined) { //условие для сквозного движения змейки
    ...
    if (!isSnakeUnit(new_unit) && !eatBomb(new_unit)) { // добавил условие про бомбу и убрал условие про конец поля
...
function eatBomb(unit) { //проверка на попадание змейки на бомбу
    var check = false;
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        livescore.innerHTML = "Ваш результат: " + score;
    }
    return check;
}
...
function createBomb() { //создание бомбы на карте
